    ...
    public class WorkUnitRowProfile : Profile
    {
        public WorkUnitRowProfile()
        {
            CreateMap<WorkUnitModel, WorkUnitRow>()
                ...
                .ForMember(dest => dest.DownloadDateTime, opt => opt.MapFrom(src => src.WorkUnit.DownloadTime))
                .ForMember(dest => dest.CompletionDateTime, opt => opt.MapFrom(src => src.WorkUnit.FinishedTime))
