...
namespace HFM.Core
{
   ...
   {
      [Test]
      public void LegacyClient_UpdateBenchmarkData_Test()
      {
         // setup
         var benchmarkCollection = new ProteinBenchmarkService();
         var database = MockRepository.GenerateMock<IUnitInfoDatabase>();
         var legacyClient = new LegacyClient { BenchmarkService = benchmarkCollection, UnitInfoDatabase = database };
         ...
         unitInfo1Clone.AddLogLineWithFrameData(new UnitRunFrameData { TimeStamp = TimeSpan.FromMinutes(0), ID = 0 });
         unitInfo1Clone.AddLogLineWithFrameData(new UnitRunFrameData { TimeStamp = TimeSpan.FromMinutes(5), ID = 1 });
         unitInfo1Clone.AddLogLineWithFrameData(new UnitRunFrameData { TimeStamp = TimeSpan.FromMinutes(10), ID = 2 });
         unitInfo1Clone.AddLogLineWithFrameData(new UnitRunFrameData { TimeStamp = TimeSpan.FromMinutes(15), ID = 3 });
