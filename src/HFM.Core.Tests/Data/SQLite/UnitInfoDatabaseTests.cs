...
namespace HFM.Core.Data.SQLite
{
   ...
   public class UnitInfoDatabaseTests
   {
      ...
      private static UnitInfo BuildUnitInfo1(int run)
      {
         ...
         unitInfo.AddWorkUnitFrame(new UnitRunFrameData { ID = 99, TimeStamp = TimeSpan.Zero });
         unitInfo.AddWorkUnitFrame(new UnitRunFrameData { ID = 100, TimeStamp = TimeSpan.FromMinutes(10) });
      ...
      private static UnitInfo BuildUnitInfo2()
      {
         ...
         // these values effect the value reported when UnitInfoLogic.GetRawTime() is called
         unitInfo.FramesObserved = 2;
         unitInfo.AddWorkUnitFrame(new UnitRunFrameData { ID = 55, TimeStamp = TimeSpan.Zero });
         unitInfo.AddWorkUnitFrame(new UnitRunFrameData { ID = 56, TimeStamp = TimeSpan.FromSeconds(1000) });
         return unitInfo;
      }
