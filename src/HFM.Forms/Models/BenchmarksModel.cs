...
public class BenchmarksModel : AsyncViewModelBase, IBenchmarksReportSource
{
    ...
    public override async Task LoadAsync()
    {
        ...
        SetFirstGraphColor();
        await RefreshSlotIdentifiers().ConfigureAwait(true);
    ...
    private async Task RefreshSlotIdentifiers()
    {
        var identifiers = await Benchmarks.GetSlotIdentifiersAsync().ConfigureAwait(true);
        var slots = Enumerable.Repeat(SlotIdentifier.AllSlots, 1)
            .Concat(identifiers.OrderBy(x => x.Name))
